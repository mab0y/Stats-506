...
```
This ocuurs beacause ChatGPT forgot to count four times ,and we also need to change it to numeric to do the max(min) operator:
```{r}
ranks <- rep(2:10, times = 4)
  ranks <- c(ranks, "Jack", "Queen", "King", "Ace")
```
So change these two lines into(11,12,13,1=="Jack", "Queen", "King", "Ace"), now it works:
```{r}
...
> Also, hands are not duplicates because they are generated one by one and cards are taken from rows of data.frame in order which are non-repeat.

> Are the names of the hands correct? (See here if you’re not familiar with poker hands.)

> No, Royal flush is forgotten within `determine_poker_hand`. It is of A, K, Q, J, 10 and all the same suit, need to add it to the top of if statement and change other statements to elif.
...
> Yes, cards are taken from rows of data.frame in order which are non-repeat. If more than 10 hands are asked to be dealta, the program will crash, so an error message is needed and the function will stop.
